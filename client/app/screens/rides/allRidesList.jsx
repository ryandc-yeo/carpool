    ...
    useCallback(() => {
      ...
      const unsubscribe = onSnapshot(configRef, async (configSnap) => {
        ...
        const passengersSnapshot = await getDocs(
          collection(db, "Sunday Passengers")
        );
        const waitlist = passengersSnapshot.docs
          .map((docSnap) => {
            const data = docSnap.data();
            if (data.driver == null) {
              return {
        ...
        <View style={styles.waitlistCard}>
          ...
          {waitlist.map((passenger, index) => (
            <Text
              ...
              style={[
                styles.personName,
                passenger.id === phoneNumber && styles.currentUserText,
              ]}
            >
              {passenger.fname} {passenger.lname} ({formatTime(passenger.time)}{" "}
              • {formatFellyDisplay(passenger.felly)})
              {passenger.id === phoneNumber && " [You]"}
            </Text>
          ))}
        </View>
